@if ($rolMembro->status == \App\Models\MembresiaMembro::STATUS_ATIVO)
    @if($rolMembro->notificacaoTransferenciaAtiva)
        <form action="{{ route('recadastramento-membro.exclusao_transferencia.cancel', $rolMembro->notificacaoTransferenciaAtiva->id) }}" 
    ...
    @else
        <a href="{{ route('recadastramento-membro.editar', $rolMembro->membro_id) }}" title="Editar"
...
@if ($rolMembro->status == \App\Models\MembresiaMembro::STATUS_INATIVO && $rolMembro->transferido == 0)
    <a href="{{ route('recadastramento-membro.reintegrar', $rolMembro->membro_id) }}" title="Reintegrar membro" class="btn btn-sm btn-secondary mr-2 btn-rounded bs-tooltip">
        <x-bx-log-in-circle />
    </a>
@endif