            ...
            ->selectRaw("
                COUNT(CASE 
                    WHEN TIMESTAMPDIFF(YEAR, mm.data_nascimento, CURDATE()) BETWEEN 0 AND 9 
                        AND mr.dt_recepcao <= '{$dataInicial}' AND mr.lastrec = 1 AND (mr.dt_exclusao IS NULL OR mr.dt_exclusao <= '{$dataInicial}') THEN mm.id
                    ELSE NULL
                END) AS Kids_X,
                COUNT(CASE 
                    WHEN TIMESTAMPDIFF(YEAR, mm.data_nascimento, CURDATE()) BETWEEN 0 AND 9 
                        AND mr.dt_recepcao <= '{$dataFinal}' AND mr.lastrec = 1 AND (mr.dt_exclusao IS NULL OR mr.dt_exclusao <= '{$dataFinal}') THEN mm.id
                    ELSE NULL
                END) AS Kids_Y,
                COUNT(CASE 
